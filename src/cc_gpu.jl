...
# ============================================================
# 第二種カニンガム鎖 (p, 2p-1, 4p-3, ...) の GPU 探索
#   鎖進行: x -> 2x - 1。第一種ファイルの build_cc_sieve_2 を使うため、
#   このファイル単独ではなく cc_cpu.jl 読み込み後に include される前提。
#   単独で使う場合は build_cc_sieve_2 が未定義なので専用ローカル版を用意。
# ============================================================

# ローカル第二種篩構築 (cc_cpu.jl 未読込時のため)
function build_cc_sieve_2_gpu(target_cc::Int)
    wl = if target_cc ≤ 9
        [2,3,5,7,11,13,17]
    elseif target_cc ≤ 11
        [2,3,5,7,11,13,17,19,23]
    ...
    eff = wheel ÷ length(wheel_n)
    println("CC$(target_cc) GPU sieve(2nd): wheel=$(wheel) residues=$(length(wheel_n)) efficiency=$(eff)")
...
# GPU kernel: 第二種カニンガム鎖カウント (x = 2x - 1)
function cc_count_kernel_2(n::Int64, target_cc::Int)::Int32
    x_lo = UInt64(n)
    x_hi = UInt64(0)
    for i in 1:target_cc
        if x_hi == 0 && x_lo <= 0x7FFFFFFFFFFFFFFF
...
# バッチ GPU フィルター (第二種)
function filter_cc_gpu_2(candidates::Vector{Int}, target_cc::Int)::Vector{Int}
    isempty(candidates) && return Int[]
    d_cand = CuArray(candidates)
    d_counts = map(n -> cc_count_kernel_2(n, target_cc), d_cand)
    counts = Array(d_counts)
    results = Int[]
    for (i, cnt) in enumerate(counts)
        if cnt == target_cc
...
# 探索本体 (逐次 GPU フィルター, 第二種)
function search_cc_gpu_2(lo::Int, hi::Int, target_cc::Int; verbose::Bool=true)
    verbose && println("=== CC$(target_cc) GPU-2nd search [$(lo), $(hi)] ===")
    t_start = time()

    if @isdefined build_cc_sieve_2
        wheel, wheel_n, mdllist = build_cc_sieve_2(target_cc)
    else
        wheel, wheel_n, mdllist = build_cc_sieve_2_gpu(target_cc)
    end
    ...
    for k in start_k:end_k
        ...
        if length(batch) >= 100_000
            verbose && println("  -> GPU batch ($(length(batch)) candidates)")
            append!(global_results, filter_cc_gpu_2(batch, target_cc))
            batch = Int[]
        end
    end
    if !isempty(batch)
    ...
    t_end = time()
    sort!(global_results)
    verbose && println("=== Done: $(length(global_results)) CC$(target_cc) in $(round(t_end - t_start, digits=2))s ===")
    for r in global_results
        println("CC$(target_cc): $(r)")
    end
    return global_results
