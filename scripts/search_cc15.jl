...
function run_search()
    ...
    println("=" ^ 70)
    println("CC$(TARGET_CC) 謗｢邏｢髢句ｧ・)
    println("蝓ｺ蠎慕ｯ・峇: 10^18 縲・Int64荳企剞")
    println("1繝舌ャ繝・= $(STEPﾃｷ10^16) ﾃ・10^16")
    println("=" ^ 70)
    ...
    while r < HI_MAX
        ...
        cumul = length(found)
        @printf "[%4d] %20d縲・20d | %6.1fs | found: %2d | total: %2d | elapsed: %ds\n" batch r r_end t_batch length(res) cumul elapsed
    ...
    println("=" ^ 70)
    println("謗｢邏｢邨ゆｺ・)
    println("邱上ヰ繝・メ謨ｰ: $batch")
    println("邱乗凾髢・ $(t_total)s ($(round(t_total/60, digits=1))蛻・")
    println("逋ｺ隕・CC$(TARGET_CC): $(length(found)) 莉ｶ")
    for f in found
